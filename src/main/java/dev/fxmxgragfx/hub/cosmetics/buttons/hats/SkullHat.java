...
public class SkullHat extends Button {


    public String getName(Player player) {
        return CC.translate("&8&lSkull Hat");
    }
    ...
    public ItemStack getButtonItem(Player player) {
        return new ItemBuilder(Material.SKULL_ITEM, 1).name(CC.translate( "&8&lSkull Hat")).data((short) 0).build();
    }

    @EventHandler
    public void clicked(Player player, int i, ClickType clickType) {
        if (player.hasPermission("hub.skullhat")) {
            if (clickType.isRightClick() || clickType.isLeftClick()) {
                ItemStack helmet = new ItemBuilder(Material.SKULL_ITEM, 1).name(CC.translate( "&8&lSkull Hat")).data((short) 0).build();
