...
public class DonatorButton extends Button {

    public String getName(Player player) {
        return CC.translate("&a&lDonator Suit");
    }
    ...
    public ItemStack getButtonItem(Player player) {
        return new ItemBuilder(Material.LEATHER_CHESTPLATE, 1).name(CC.translate("&aDonator Suit")).color(convert("2")).build();
    }

    @EventHandler
    public void clicked(Player player, int i, ClickType clickType) {
        if (player.hasPermission("hub.donatorsuit")) {
            if (clickType.isRightClick() || clickType.isLeftClick()) {
                ItemStack helmet = new ItemBuilder(Material.LEATHER_HELMET, 1).name(CC.translate("&aDonator Suit")).color(convert("2")).build();
                ItemStack chestplate = new ItemBuilder(Material.LEATHER_CHESTPLATE, 1).name(CC.translate("&aDonator Suit")).color(convert("2")).build();
                ItemStack leggings = new ItemBuilder(Material.LEATHER_LEGGINGS, 1).name(CC.translate("&aDonator Suit")).color(convert("2")).build();
                ItemStack boots = new ItemBuilder(Material.LEATHER_BOOTS, 1).name(CC.translate("&aDonator Suit")).color(convert("2")).build();
