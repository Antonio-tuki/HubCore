...
public class OwnerShipButton extends Button {

    public String getName(Player player) {
        return CC.translate("&6&lOwnerShip Suit");
    }
    ...
    public ItemStack getButtonItem(Player player) {
        return new ItemBuilder(Material.LEATHER_CHESTPLATE, 1).name(CC.translate( "&6OwnerShip Suit")).color(convert("4")).build();
    }

    @EventHandler
    public void clicked(Player player, int i, ClickType clickType) {
        if (player.hasPermission("hub.ownersuit")) {
            if (clickType.isRightClick() || clickType.isLeftClick()) {
                ItemStack helmet = new ItemBuilder(Material.LEATHER_HELMET, 1).name(CC.translate( "&6Owner Suit")).color(convert("4")).build();
                ItemStack chestplate = new ItemBuilder(Material.LEATHER_CHESTPLATE, 1).name(CC.translate( "&6Owner Suit")).color(convert("4")).build();
                ItemStack leggings = new ItemBuilder(Material.LEATHER_LEGGINGS, 1).name(CC.translate( "&6Owner Suit")).color(convert("4")).build();
                ItemStack boots = new ItemBuilder(Material.LEATHER_BOOTS, 1).name(CC.translate( "&6Owner Suit")).color(convert("4")).build();
