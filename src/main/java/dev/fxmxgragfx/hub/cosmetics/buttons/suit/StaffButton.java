...
public class StaffButton extends Button {
    ...
    public String getName(Player player) {
        return CC.translate("&3&lStaff Suit");
    }
    ...
    public ItemStack getButtonItem(Player player) {
        return new ItemBuilder(Material.LEATHER_CHESTPLATE, 1).name(CC.translate( "&3Staff Suit")).color(convert("3")).build();
    }

    @EventHandler
    public void clicked(Player player, int i, ClickType clickType) {
        if (player.hasPermission("hub.staffsuit")) {
            if (clickType.isRightClick() || clickType.isLeftClick()) {
                ItemStack helmet = new ItemBuilder(Material.LEATHER_HELMET, 1).name(CC.translate( "&3Staff Suit")).color(convert("3")).build();
                ItemStack chestplate = new ItemBuilder(Material.LEATHER_CHESTPLATE, 1).name(CC.translate( "&3Staff Suit")).color(convert("3")).build();
                ItemStack leggings = new ItemBuilder(Material.LEATHER_LEGGINGS, 1).name(CC.translate( "&3Staff Suit")).color(convert("3")).build();
                ItemStack boots = new ItemBuilder(Material.LEATHER_BOOTS, 1).name(CC.translate( "&3Staff Suit")).color(convert("3")).build();
